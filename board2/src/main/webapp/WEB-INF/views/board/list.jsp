					...
					<form id='searchForm' action="/board/list" method="get">
						<select name='type'>
							...
							<option value="TW"
								...
								or 작성자</option>
							<option value="WC"
								<c:out value="${pageMaker.cri.type eq 'WC' ? 'selected':'' }"/>>내용
								or 작성자</option>
							<option value="TCW"
				...
				<c:forEach items="${list}" var="board">
					<tr>
						...
						<td><fmt:formatDate pattern="yyyy-MM-dd"
								value="${board.cdate}" /></td>
						<td><fmt:formatDate pattern="yyyy-MM-dd"
								value="${board.udate}" /></td>
					</tr>
				</c:forEach>
			</table>
						...
						$(".paginate_button a").on(
								...
								function(e) {
									...
									actionForm.find("input[name='pageNum']")
											.val($(this).attr("href")); //현재 선택한 번호의 href값을 가져와 pageNum값으로 세팅한다.
									actionForm.submit();
								});
						$(".move")
								.on(
										...
										function(e) {
											...
											actionForm
													.append("<input type='hidden' name='bno' value='"
															+ $(this).attr(
																	"href")
															+ "'>"); //현재 선택한 태그의 href 속성에 적혀져 있는 값을 가져와 bno값으로 세팅한다.
											actionForm.attr("action",
													"/board/get"); //action 값 변경
											actionForm.submit();
										});
